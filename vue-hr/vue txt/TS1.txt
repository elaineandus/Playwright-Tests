...
test('TS_1', async ({ page }) => {
   ...
   //end of policy changes
    ...
    await page.locator('td:nth-child(3) > .form-control').first().fill('2022-10-26T07:49');
    await page.locator('td:nth-child(3) > .form-control').last().fill('2022-10-26T18:00');
    ...
    try{ 
        ...
        await expect(Workhrscell).toContainText('9');
    } catch (error){await test.fail();}
    try{ 
    ...
    try{ 
        ...
        await expect(Overbreakcell).toContainText('60');
    } catch (error){await test.fail();}
    try{ 
        const OTcell = page.locator(`#tblDTRFixed > tbody > tr:nth-child(1) > td:nth-child(12)`);
        await expect(OTcell).toContainText('60');
    } catch (error){await test.fail();}    
    try{ 
        const NDcell = page.locator(`#tblDTRFixed > tbody > tr:nth-child(1) > td:nth-child(13)`);

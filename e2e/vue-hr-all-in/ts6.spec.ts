...
// Break:
await page.getByLabel('Calculate Overbreak').uncheck();
...
await page.getByLabel('No Pay When Absent Before or After Holiday').uncheck();
   ...
   await page.locator('td:nth-child(3) > .form-control').last().fill('2022-10-27T19:00');
   ...
   try { 
         ...
         await expect(Statuscell).toHaveText('Pending RD OT');
   } catch (error) {
         await test.fail();
   };
   ...
   try {
         ...
         await expect(Workhrscell).toHaveText('7');
   } catch (error) {
         await test.fail();
   };

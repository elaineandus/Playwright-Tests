...
test('test all', async ({ page }) => {
  ...
  await page.getByRole('option', { name: 'Client 1 TIN - 123456789' }).click();
  ...
  await expect(discountTotal).toHaveText('3,268.18');
  } catch (error) {
    await test.fail();
  }
  ...
  await expect(totalCell).toHaveText('15,600.74');
  } catch (error) {
    await test.fail();
  }

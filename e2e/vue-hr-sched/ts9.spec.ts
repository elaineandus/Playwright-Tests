...
test('test scenario 9', async ({ page }) => {
   ...
   await page.locator('div').filter({ hasText: 'OT Based on Required Hours per Day' }).locator('#NoOvertimeIfNotOvertheSchedule').last().check();
   ...
   try {
         ...
         await expect(Workhrscell).toHaveText('7.55');
   } catch (error) {
         await test.fail();
   };

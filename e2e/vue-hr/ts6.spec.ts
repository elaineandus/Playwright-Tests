...
test('test scenario 6', async ({ page }) => {
...
await page.getByLabel('No Pay When Absent Before or After Holiday').uncheck();
   ...
   await page.locator('td:nth-child(3) > .form-control').last().fill('2022-10-29T19:00');
   ...
   try { 
         ...
         await expect(Statuscell).toContainText('Pending RD OT');
   } catch (error) {
         await test.fail();
   };
   ...
   try {
         ...
         await expect(Workhrscell).toContainText('8');
   } catch (error) {
         await test.fail();
   };
   ...
   try {
         const NDFcell = page.locator(`#tblDTRFixed > tbody > tr:nth-child(4) > td:nth-child(13)`);

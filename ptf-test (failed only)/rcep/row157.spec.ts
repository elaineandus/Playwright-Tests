...
test('should check correct tariff reduction schedule', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Search' }).fill('ex2 3808.59.19.900');
  await page.locator('a').filter({ hasText: 'ex2 3808.59.19.900 Other' }).first().click();
  
  await page.getByRole('button', { name: 'Search' }).click();

  await page.getByRole('link', { name: 'Contains Extraction' }).click();
  await page.getByRole('cell', { name: 'ex2 3808.59.19.900' }).getByText('ex2 3808.59.19.900').click();

  // Select the table cell with the code
  const cell = await page.locator('#exConcessionRate td:nth-child(1)');
  await expect(cell).toContainText('ex2 3808.59.19.900');
 ...
 */
  ...
  await expect(k23Cell).toContainText('3');
  } catch (error) {
    await test.fail();
  }
  ...
  await expect(k24Cell).toContainText('3');
  } catch (error) {
    await test.fail();
  }
  ...
  await expect(k27Cell).toContainText('3');
  } catch (error) {
    await test.fail();
  }
